...
export class AccountService {
  ...
  async createAccount(data: CreateAccountRequest): Promise<AccountDTO> {
    if ((await this._accountRepository.findByEmail(data.email)) != null)
      throw new BadRequestException('email aready exist!');
    if (
      data.roleId != null &&
      (await this._roleService.getById(data.roleId)) == null
    )
      throw new BadRequestException('role doesnot exist!');
  ...
  async getAllAccounts(
    ...
    const [items, count] =
      await this._accountRepository.findWithOption(pageOptionsDto);
