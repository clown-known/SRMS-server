...
export class RouteService {
  ...
  async save(createRouteDTO: CreateRouteDTO): Promise<CreateRouteDTO> {
    const startPointId =
      typeof createRouteDTO.startPointId === 'object'
        ? createRouteDTO.startPointId.id
        : createRouteDTO.startPointId;
    const endPointId =
      typeof createRouteDTO.endPointId === 'object'
        ? createRouteDTO.endPointId.id
        : createRouteDTO.endPointId;

    const startPoint = await this.pointRepository.findOne({
      where: { id: startPointId },
    });
    const endPoint = await this.pointRepository.findOne({
      where: { id: endPointId },
    });
    ...
    const resultDTO = plainToInstance(CreateRouteDTO, savedRoute);
    resultDTO.startPointId = plainToInstance(PointDTO, savedRoute.startPoint);
    resultDTO.endPointId = plainToInstance(PointDTO, savedRoute.endPoint);
  ...
  ): Promise<{ data: RouteDTO[]; meta: PageMetaDto }> {
  ...
  async updateRoute(
    ...
    if (updateRouteDTO.startPointId) {
      const startPoint = await this.pointRepository.findOne({
        where: { id: updateRouteDTO.startPointId.id },
      });
      if (!startPoint) {
        throw new BadRequestException('Invalid startPoint ID');
      }
    ...
    if (updateRouteDTO.endPointId) {
      const endPoint = await this.pointRepository.findOne({
        where: { id: updateRouteDTO.endPointId.id },
      });
      if (!endPoint) {
        throw new BadRequestException('Invalid endPoint ID');
      }
